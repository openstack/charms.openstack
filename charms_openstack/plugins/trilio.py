...
def get_trilio_charm_instance(release=None, package_type='deb', *args,
    ...
    :param release: String representing release wanted. Should be of the form
                    '<openstack_release>_<trilio_release>' eg 'queens_4.0'
    :type release: str
    ...
    if release is None:
        # If release is None then select the class(es) which supports the most
        # recent OpenStack release, from within this set select the class that
        # supports the most recent Trilio release.
        ...
        if (os_release_index <
            ...
            raise RuntimeError(
                ...
                "is {} release".format(os_release, known_os_releases[0]))
        else:
            known_trilio_releases = []
            # Search through the dictionary of registered charm classes
            # looking for the most recent group which can support `os_release`
            for known_os_release in reversed(known_os_releases):
                if (os_release_index >=
                        os_utils.OPENSTACK_RELEASES.index(known_os_release)):
                    trilio_classes = _trilio_releases[known_os_release]
                    known_trilio_releases = sorted(trilio_classes.keys())
                    break
            # Search through the dictionary of registered charm classes
            # that support `known_os_release` onwards and look for the class
            # which supports the most recent trilio release which is <=
            # `trilio_release`
            for known_trilio_release in reversed(known_trilio_releases):
                if known_trilio_release <= trilio_release:
                    cls = trilio_classes[known_trilio_release][package_type]
...
def select_trilio_release():
    ...
    that is installed. If it is not installed fall back to checking the charm
    `triliovault-pkg-source` option.
    ...
    if os_release_version is None:
        try:
            ...
            singleton = get_trilio_charm_instance()
...
class TrilioVaultCharmMixin():
    ...
    def run_trilio_upgrade(self, interfaces_list=None):
        ...
        _configure_triliovault_source()
        target_charm = get_trilio_charm_instance(
            new_release)
